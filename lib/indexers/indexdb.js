...
class IndexDB extends EventEmitter {
  ...
  constructor(options) {
    ...
    for (const indexer of this.node.indexers) {
      const plugin = require('./' + indexer);
      this.indexer.push(plugin.indexBlock);
      this.unindexer.push(plugin.unindexBlock);
    }
  ...
  _bind() {
    this.node.on('connect', async (entry, txs) => {
      try {
        await this.indexBlock(entry, txs);
      } catch (e) {
        this.emit('error', e);
      }
    });

    this.node.on('disconnect', async (entry) => {
      try {
        await this.unindexBlock(entry);
      } catch (e) {
        this.emit('error', e);
      }
    });
  ...
  async indexBlock(entry, txs) {
    const unlock = await this.lock.lock();
    try {
      return await this._indexBlock(entry, txs);
    } finally {
      unlock();
    }
  }
  ...
  async _indexBlock(entry, txs) {
    for (const f of this.indexers)
      f(entry, txs);
  }
  ...
  async unindexBlock(entry) {
    const unlock = await this.lock.lock();
    try {
      return await this._unindexBlock(entry);
    } finally {
      unlock();
    }
  }
  ...
  async _unindexBlock(entry) {
    for (const u of this.unindexers)
      u(entry);
  }
}
