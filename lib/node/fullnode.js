...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    if (this.config.bool('index-tx') ||
        this.config.bool('index-address')) {
      this.use(require('bindex'));
      this.index = this.require('bindex');
    }
  ...
  async getCoinsByAddress(addrs) {
    ...
    const index = await this.index.getCoinsByAddress(addrs);
    const out = [];
  ...
  async getMetaByAddress(addrs) {
    ...
    if (this.index) {
      const index = await this.index.getMetaByAddress(addrs);
  ...
  async getMeta(hash) {
    ...
    if (this.index)
        return this.index.getMeta(hash);
